...
补充健康约束：
- **跑步 6km**：每日 06:30-07:15，需至少 1 小时（含热身拉伸）
- **轻食**：正常就餐时间
- **饭后不能马上跑步**，至少等 30 分钟
- 健康时间块不可被其他任务挤占
...
**时间块任务（Time Block）**：需要固定时间段、有明确起止时间的任务。写入 events.json + 日报「时间安排」表。
典型：会议、上课、面试、深度工作、运动、吃饭、通勤。

**TODO 任务**：只需要「今天做」、不需要固定时间的任务。写入日报「今日优先」列表。
...
1. 固定会议 / 课程：不可移动，其他任务让位
2. 健康/运动：硬约束，不可被挤占
3. 有 deadline 的任务：优先安排
4. 学习时间：尽量保证
5. 杂务：有空就做
6. 娱乐放松：适度
...
## Google Calendar 联动（可选镜像）

**默认不再自动同步到 Google Calendar**，除非用户明确要求。原因：
- events.json 才是网站真实数据源
- 多一份同步逻辑 = 多一份不一致风险
- Google Calendar 适合手机系统日历提醒，events.json 适合网页可视化

如需镜像，流程：
1. 写入 events.json（用 CLI）
2. 提示用户用 `mcp_google-calendar_create-event` 同步到 Google Calendar
3. 或者 Agent 同时调用两者（高频场景才值得）

账号：`xinyuzheng31`（主）/ `personal`（个人）
...
| 文档 | 何时读取 |
|------|---------|
| [references/scheduling.md](references/scheduling.md) | 排日程、调度优先级、健康约束的完整规范 |
